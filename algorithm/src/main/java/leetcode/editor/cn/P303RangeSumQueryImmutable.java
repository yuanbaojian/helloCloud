...
// 
// Related Topics 设计 数组 前缀和 👍 454 👎 0
...
public class P303RangeSumQueryImmutable{
    public static void main(String[] args) {
        ...
        NumArray solution = new P303RangeSumQueryImmutable().new NumArray(array);
        final int i = solution.sumRange(2, 5);
...
class NumArray {
    ...
    public NumArray(int[] nums) {
        rawArray = nums;
        sumArray = new int[rawArray.length];
        for (int i = 0; i < rawArray.length; i++) {
            if(i==0){
                sumArray[0] = rawArray[0];
            } else {
                sumArray[i] = rawArray[i] + sumArray[i-1];
            }
        }
    }
